export default class Visualizer {
    ...
    initBalls(count) {
        ...
        // ?????????????????????????
        const colors = [
            'rgba(0, 122, 255, 0.6)', 'rgba(52, 199, 89, 0.6)', 
            'rgba(255, 59, 48, 0.6)', 'rgba(255, 149, 0, 0.6)', 'rgba(175, 82, 222, 0.6)' 
        ];
    ...
    triggerNoteImpact() {
        ...
        this.balls.forEach(ball => {
            // 20% ??????“??”
            if (Math.random() < 0.2) {
                ball.targetScale = 1.4; // ??????????
                ball.isStopped = false;
                
                // --- ???????????? ---
                // ?????0 ~ 3.0 (???? 3.0 ?“????”??????????)
                const force = Math.random() * 3.0; 
                ...
                setTimeout(() => { ball.targetScale = 1.0; }, 150);
            }
        });
    }
    // [??] ??????
    triggerZoneAttraction(targetX, targetY) {
        // 1. ????????? (????3?)
        const zoneWidth = this.width / 3;
        
        // 2. ?????(??)????? (0:?, 1:?, 2:?)
        // ??????????????????2
        let targetZone = Math.floor(targetX / zoneWidth);
        if (targetZone > 2) targetZone = 2; 

        this.balls.forEach(ball => {
            // 3. ???????????
            let ballZone = Math.floor(ball.x / zoneWidth);
            if (ballZone > 2) ballZone = 2;

            // 4. ????????????
            if (ballZone === targetZone) {
                // ????
                ball.isStopped = false;
    ...
    updatePhysics() {
        ...
        for (let i = 0; i < this.balls.length; i++) {
            ...
            if (ball.x + ball.r > this.width) { ball.x = this.width - ball.r; ball.vx *= -1; }
            if (ball.y - ball.r < 0) { ball.y = ball.r; ball.vy *= -1; }
            if (ball.y + ball.r > this.height) { ball.y = this.height - ball.r; ball.vy *= -1; }
        }
